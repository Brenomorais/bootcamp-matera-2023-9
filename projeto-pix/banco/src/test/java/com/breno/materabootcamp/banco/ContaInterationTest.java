...
public class ContaInterationTest {
	...
	@Test
	public void deveRettornOKCriarUmBanco() {
		
		String requestBanco = "{\"nome\": \"Banco Spring\"}";
		
		Banco bancoResponse = RestAssured
				.given().log().all()
				...
				.extract()
				.jsonPath().getObject(".", Banco.class);
		
		if(bancoResponse != null) {
			this.codigoBanco = bancoResponse.getCodigo();
			Assertions.assertEquals("Banco Spring", bancoResponse.getNome());
		}				
	...
	public void deveRetornarOKCriarUmaConta() {
		...
		deveRettornOKCriarUmBanco();
		
		Map<String, Object> resquetConta = new HashMap<>();
		resquetConta.put("agencia", "123");
		resquetConta.put("numeroConta", "456");
		resquetConta.put("nome", "User");
		resquetConta.put("cpf", "00000000000");
		resquetConta.put("codigo", this.codigoBanco);
	...
	void deveRetonarErroDeSaldoInsuficiente() {
		...
		if (contasReponse.size() == 2) {
			...
			RestAssured.given().log().all()
				...
				.then().statusCode(HttpStatus.CONFLICT.value()).extract();
		}
	}
